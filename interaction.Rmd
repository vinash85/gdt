...
#' Please refer Kryuchkova-Mostacci, N., & Robinson-Rechavi, M. (2017). A benchmark of gene expression tissue-specificity metrics. Briefings in Bioinformatics, 18(2), 205–214. https://doi.org/10.1093/bib/bbw008
...
tls.genes = c("MS4A1", "CD40", "CXCR5", "CXCL13", "CCR7", "CCL19",  "CD8B", "PDCD1",  "CTLA4","HAVCR2", "CD274", "PDCD1LG2")
...
avi.dt = trg.crispr.cor$cor[c("TRDV2", "TRDV1", "TRDV3"),] %>% t  %>% data.table %>%
	.[,gene:= colnames(trg.crispr.cor$cor)] %>%
...
avi.sel=avi.dt[intersect(
	...
	order(avi.dt$TRDV1,decreasing=F)[1:200] )]
p = get.enrich(genes=avi.sel$gene, universe=avi.dt$gene)
ggsave(file="results/.figs/gdt/pathway.cor.trdv2.neg.trdv1.neg.pdf",p)


aa = fit.loess(avi.dt$TRDV1, avi.dt$TRDV3)
inx.pos = (avi.dt$TRDV3 - aa$fitted) %>% order(decreasing=T) %>% 
.[1:40]
inx.neg = (aa$fitted- avi.dt$TRDV3 ) %>% order(decreasing=T) %>% 
.[1:30]
# avi.dt.sub = avi.dt[inx]
avi.dt.sub = avi.dt[unique(c(
	order(avi.dt$TRDV1,decreasing=F)[1:30],
	order(avi.dt$TRDV3,decreasing=F)[1:30],
	order(avi.dt$TRDV1,decreasing=T)[1:40],
	order(avi.dt$TRDV3,decreasing=T)[1:40], 
	inx.pos,
	inx.neg
	))]
p.sc.volcano = ggplot(data=avi.dt, aes(x=TRDV1, y= TRDV3))  + 
geom_point(alpha=0.5) +
theme_minimal(base_size = 12) + theme(legend.position = "bottom") +
geom_text_repel(
	data = avi.dt.sub,
	 aes(x = TRDV1, y = TRDV3, label = gene),    
	 size = 3,
    box.padding = unit(0.35, "lines"),
    point.padding = unit(0.3, "lines")
    )  

ggsave(file="results/.figs/gdt/gene.cor.trdv3.trdv1.pdf", p.sc.volcano, width=16, height=10)


avi.sel=avi.dt[TRDV1>0.5 & TRDV3 > 0.4]
p = get.enrich(genes=avi.sel$gene, universe=avi.dt$gene)
ggsave(file="results/.figs/gdt/pathway.cor.trdv3.pos.trdv1.pos.pdf",p)

try({
p = get.enrich(genes=avi.dt[inx.pos]$gene, universe=avi.dt$gene)
ggsave(file="results/.figs/gdt/pathway.cor.trdv3.pos.trdv1.neg.pdf",p)
})

try({
p = get.enrich(genes=avi.dt[inx.neg]$gene, universe=avi.dt$gene)
ggsave(file="results/.figs/gdt/pathway.cor.trdv3.neg.trdv1.pos.pdf",p)
})

avi.sel=avi.dt[intersect(
	order(avi.dt$TRDV3,decreasing=F)[1:200], 
	order(avi.dt$TRDV1,decreasing=F)[1:200] )]
p = get.enrich(genes=avi.sel$gene, universe=avi.dt$gene)
ggsave(file="results/.figs/gdt/pathway.cor.trdv3.neg.trdv1.neg.pdf",p)



library(ggplot2)
library(ggrepel)
avi.dt = trg.crispr.cor$cor[c("TRGV2", "TRGV9", "TRGV10"),] %>% t  %>% data.table %>%
	.[,gene:= colnames(trg.crispr.cor$cor)] %>%
	.[!(is.na(TRGV2) | is.na(TRGV2))]
aa = fit.loess(avi.dt$TRGV2, avi.dt$TRGV9)
inx.pos = (avi.dt$TRGV9 - aa$fitted) %>% order(decreasing=T) %>% 
.[1:70]
inx.neg = (aa$fitted- avi.dt$TRGV9 ) %>% order(decreasing=T) %>% 
.[1:70]
# avi.dt.sub = avi.dt[inx]
avi.dt.sub = avi.dt[unique(c(
	order(avi.dt$TRGV2,decreasing=F)[1:30],
	order(avi.dt$TRGV9,decreasing=F)[1:30],
	order(avi.dt$TRGV2,decreasing=T)[1:40],
	order(avi.dt$TRGV9,decreasing=T)[1:40], 
	inx.pos,
	inx.neg
	))]
p.sc.volcano = ggplot(data=avi.dt, aes(x=TRGV2, y= TRGV9))  + 
geom_point(alpha=0.5) +
theme_minimal(base_size = 12) + theme(legend.position = "bottom") +
geom_text_repel(
	data = avi.dt.sub,
	 aes(x = TRGV2, y = TRGV9, label = gene),    
	 size = 3,
    box.padding = unit(0.35, "lines"),
    point.padding = unit(0.3, "lines")
    )  

ggsave(file="results/.figs/gdt/gene.cor.trgv9.trgv2.pdf", p.sc.volcano, width=16, height=10)

avi.sel=avi.dt[intersect(
	order(avi.dt$TRGV9,decreasing=T)[1:200], 
	order(avi.dt$TRGV2,decreasing=T)[1:200] )]
p = get.enrich(genes=avi.sel$gene, universe=avi.dt$gene)
ggsave(file="results/.figs/gdt/pathway.cor.trgv9.pos.trgv2.pos.pdf",p)

try({
p = get.enrich(genes=avi.dt[inx.pos]$gene, universe=avi.dt$gene)
ggsave(file="results/.figs/gdt/pathway.cor.trgv9.pos.trgv2.neg.pdf",p)
})

try({
p = get.enrich(genes=avi.dt[inx.neg]$gene, universe=avi.dt$gene)
ggsave(file="results/.figs/gdt/pathway.cor.trgv9.neg.trgv2.pos.pdf",p)
})

avi.sel=avi.dt[intersect(
	order(avi.dt$TRGV9,decreasing=F)[1:200], 
	order(avi.dt$TRGV2,decreasing=F)[1:200] )]
p = get.enrich(genes=avi.sel$gene, universe=avi.dt$gene)
ggsave(file="results/.figs/gdt/pathway.cor.trgv9.neg.trgv2.neg.pdf",p)
