---
title: "Determine co-localization in scRNA "
...
```{r}
aa = sapply(exps, function(tt) tt$meta.data %>% dim)
...
psuedobulk.exps = mclapply(files, function(file){
  sco = readRDS(file)
  label = file %>% basename %>% gsub("_res.rds", "", .)
  genes.curr = rownames(sco$RNA)
  temp = create.psuedobulk(sco$RNA@assays$RNA@data, labels = sco$RNA@meta.data$seurat_clusters)
  list(label =label, meta.data = sco$RNA@meta.data, exp = temp)
}, mc.cores = 48
)

ref.data <- SingleR::MonacoImmuneData()

get.SingleR.annotation = function (exp.curr, ref.data,  ...) 
{
    require(SingleR)
    singleR.annotations = list()
    singleR.annotations$fine <- SingleR(test = exp.curr, 
        ref = ref.data, labels = ref.data$label.fine, ...)
    singleR.annotations$main <- SingleR(test = exp.curr, 
        ref = ref.data, labels = ref.data$label.main, ...)
    singleR.annotations
}

singleR.annot = mclapply(psuedobulk.exps, function(curr){
  temp = get.SingleR.annotation(curr$exp, ref.data=ref.data)
  list(label =curr$label, meta.data = curr$meta.data, annot = temp)
}, mc.cores = 48
)

# files = list.files(pattern="*.res.rds", full.names = T, path = "~/liulab_home/data/links/data/", recursive = T)
all.files = list.files(pattern="*.res.rds", full.names = T, path = "/liulab/zzeng/Tigger/static", recursive = T)
refs = readRDS("~/liulab_home/data/single_cell/singleR.dataset.RDS")
singleR.cell.annot = mclapply(files, function(file){
  sco = readRDS(file)
  label = file %>% basename %>% gsub("_res.rds", "", .)
  exp.curr = sco$RNA@assays$RNA@data
  cluster.curr = sco$RNA$seurat_clusters
  singleR.annots = sapply(names(refs), function(tt) get.SingleR.annotation(exp.curr, ref.data=refs[[tt]]$ref.se, de.method = "t"),
                          USE.NAMES = T,simplify = F)
  saveRDS(file = sprintf("~/liulab_home/data/single_cell/singleR/%s.rds", label), singleR.annots)
  list(label =label, singleR.annots = singleR.annots)
}, mc.cores = 60
)
saveRDS(file="~/liulab_home/data/single_cell/all.datasets.singleR.cell.annot.Rds", object = singleR.cell.annot)

singleR.cell.annot = list() 
for(file  in files[1:2]){
  sco = readRDS(file)
  label = file %>% basename %>% gsub("_res.rds", "", .)
  exp.curr = sco$RNA@assays$RNA@data[,1:1000]
  cluster.curr = sco$RNA$seurat_clusters[1:1000]
  singleR.annots = sapply(names(refs)[1], function(tt) get.SingleR.annotation(exp.curr, ref.data=refs[[tt]]$ref.se,   method="cluster", clusters=cluster.curr),
                          USE.NAMES = T,simplify = F)
  singleR.cell.annot[[label]] = singleR.annots
}  


saveRDS(file="~/liulab_home/data/single_cell/all.datasets.singleR.cell.annot.1_40.Rds", object = singleR.cell.annot)

files.private = list.files(pattern="*.res.rds", full.names = T, path = "~/liulab_home/data/links/data_private/", recursive = T)
singleR.cell.annot.private = mclapply(files.private, function(file){
  sco = readRDS(file)
  label = file %>% basename %>% gsub("_res.rds", "", .)
  exp.curr = sco$RNA@assays$RNA@data
  refs = readRDS("~/liulab_home/data/single_cell/singleR.dataset.RDS")
  singleR.annots = sapply(names(refs), function(tt) get.SingleR.annotation(exp.curr, ref.data=refs[[tt]]$ref.se, de.method = "t"),
                          USE.NAMES = T)
  list(label =label, meta.data = sco$RNA@meta.data, singleR.annots = singleR.annots)
}, mc.cores = 6
)
saveRDS(file="~/liulab_home/data/single_cell/all.datasets.singleR.cell.annot.private.Rds", object = singleR.cell.annot.private)


all.files = list.files(pattern="*.res.rds", full.names = T, path = "/liulab/zzeng/Tigger/static", recursive = T)
singleR.cluster.annot = mclapply(all.files, function(file){
  sco = readRDS(file)
  label = file %>% basename %>% gsub("_res.rds", "", .)
  exp.curr = sco$RNA@assays$RNA@data
  cluster.curr = sco$RNA$seurat_clusters
  refs = readRDS("~/liulab_home/data/single_cell/singleR.dataset.RDS")
  singleR.annots = sapply(names(refs), function(tt) 
    get.SingleR.annotation(exp.curr, ref.data=refs[[tt]]$ref.se, de.method = "t", method="cluster", clusters=cluster.curr),
                          USE.NAMES = T)
  list(label =label, meta.data = sco$RNA@meta.data, singleR.annots = singleR.annots)
}, mc.cores = 60
)
saveRDS(file="~/liulab_home/data/single_cell/all.datasets.singleR.cluster.annot.Rds", object = singleR.cluster.annot)
