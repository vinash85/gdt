...
FeaturePlot(pbmc.combined,  reduction = "tsne", features = c("gdt", "NLRC5", "TRDC", "TRGC1", "TRGC2","CD3E" ,  "CD8A", "GNLY", "NKG7", "PRF1", "GZMB", "GZMK", "GZMH")) %>% 
...
## Assocation between different immmune infiltrations
```{r}
tcga.singler = readRDS("~/liulab_home/data/tcga/tcga.singler.Rds")
project.exp = readRDS("~/liulab_home/data/tcga/TCGA_gdc/tcgabiolinks/all.tcga.expression.RDS")

aa = WGCNA::corAndPvalue(x=tcga.singler["T_cell:gamma-delta",], t(tcga.singler), method="spearman")
avi.dt = data.table(cor=aa$cor[1,], p=aa$p[1,], label=rownames(tcga.singler))
p = EnhancedVolcano::EnhancedVolcano(avi.dt,
	lab = rownames(tcga.singler),
	x =  aa$cor[1,],
	y =  aa$p[1,],
	pCutoff = 1E-200,
	FCcutoff = .6,
