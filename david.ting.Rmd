...
## marker genes
```{r}
calcualte.eis.scra = function(sco, eis){
  tryCatch({
  (sco@assays$RNA@data %>% as.matrix)->scale.exp
  genes.overlap = intersect(toupper(rownames(scale.exp)), toupper(eis$Gene.symbol))
  cor(x=scale.exp[genes.overlap, ] , y=eis[match(genes.overlap,Gene.symbol)]$FC_treatment, method ="spearman", use="pairwise.complete.obs")
  }, error=function(e) NA)
}
eis = fread("~/liulab_home/data/xiaoman/xmwang/oxphos_proj/loading_data/LP_data/degene_72_pos.csv")

# aa = calcualte.eis.scra(all.scrna.sel[[1]], eis=eis)
# bcc.oxphos.score<- getCor(scale.bcc.oxphos,columnLoc = "FC_treatment",
# method = "spearman")
sco.eis = calcualte.eis.scra(sco,  eis=eis)
sco$eis= sco.eis[,1]
FeaturePlot(sco,features = "eis")
```

Use RCTD to score each roi 
